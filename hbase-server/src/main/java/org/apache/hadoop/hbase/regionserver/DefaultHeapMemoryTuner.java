 ...
 * by <i>step</i> and increase the other by same amount. If none of them is sufficient we do
 ...
 * be specified in config by <i>hbase.regionserver.heapmemory.autotuner.lookup.periods</i>. Also
 ...
 * We maintain a parameter <i>decayingAvgTunerStepSize</i> which is sum of past tuner steps with
 ...
 * <i>hbase.regionserver.heapmemory.autotuner.step.max</i>. If in a particular tuning operation the
 * step direction is opposite to what indicated by <i>decayingTunerStepSizeSum</i> we decrease the
...
class DefaultHeapMemoryTuner implements HeapMemoryTuner {
  ...
  public TunerResult tune(TunerContext context) {
    ...
    if (
      prevTuneDirection == StepDirection.NEUTRAL && newTuneDirection != StepDirection.NEUTRAL
      ...
      (newTuneDirection == StepDirection.INCREASE_MEMSTORE_SIZE && decayingTunerStepSizeSum < 0)
    ...
    if (
      totalOnheapFlushCount == 0 && offheapMemstore
        && newTuneDirection == StepDirection.INCREASE_BLOCK_CACHE_SIZE
    ) {
...
if(KnobRuntime.check(java.util.UUID.fromString("e27669fd-a32b-314f-a909-10e44f2382e0"))) {
try {
...
if(KnobRuntime.check(java.util.UUID.fromString("ee853b53-4f0d-369c-b7a1-3fc92a3b6f8b"))) {
return;
}
    this.conf = conf;
